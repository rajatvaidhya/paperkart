    ...
    try {
      const response = await fetch(
        "http://localhost:5000/api/items/addAdminItems",
        {
          ...
          body: JSON.stringify({ title:title, description:description, price:price, image:image, category:category, subCategory:subCategory, quantity:quantity }),
        }
      );
      const data = await response.json();
    } catch (error) {
    ...
    <>
      ...
      <form onSubmit={handleSubmit}>
        <div className="main-admin-container">
          <h1 style={{color:'black'}}>Add <span style={{color:'red'}}>Products</span></h1>
          ...
          <div className="add-container">
            ...
            <div className="form-add-container">
              ...
              <button className="btn" style={{color:'white'}} type="submit">Add</button>
            </div>
          </div>
        </div>
      </form>
